  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold text-slate-900">Equipment Movements</h1>
          <p className="text-slate-500">Track dispatch and returns across all projects.</p>
        </div>
        <button 
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="px-6 py-4 font-semibold text-slate-700">Project / Client</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                <tr><td colSpan={5} className="p-8 text-center text-slate-500">Loading movements...</td></tr>
              ) : movements?.length === 0 ? (
                <tr><td colSpan={5} className="p-8 text-center text-slate-500">No movements found.</td></tr>
              ) : (
                movements?.map((move) => (
                  <tr key={move.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${
                        ...
                        {move.type === 'OUT' ? 'DISPATCH' : 'RETURN'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-slate-600 font-medium">
                      {format(new Date(move.date), 'dd MMM yyyy')}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <span className="font-bold text-slate-900">{move.itemsCount}</span> items
                    </td>
                  </tr>
                ))
              )}
...
function CreateMovementForm({ onCancel }: { onCancel: () => void }) {
  ...
  const { register, control, handleSubmit, watch, formState: { errors } } = useForm<z.infer<typeof createMovementSchema>>({
    ...
    defaultValues: {
      type: 'OUT',
      date: new Date().toISOString().split('T')[0] as any, // Simple date handling
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-display font-bold text-slate-900">New Movement</h2>
          <p className="text-slate-500">Record equipment dispatch or return.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="col-span-full md:col-span-2 flex gap-4">
              <label className="flex-1 cursor-pointer">
                ...
                <div className="p-4 rounded-xl border-2 border-slate-200 peer-checked:border-amber-500 peer-checked:bg-amber-50 transition-all text-center">
                  <div className="font-bold text-slate-700 peer-checked:text-amber-700 mb-1">DISPATCH (To Site)</div>
                  <div className="text-xs text-slate-500">Sending equipment to a project</div>
                </div>
              </label>
              <label className="flex-1 cursor-pointer">
                <input 
                ...
                <div className="p-4 rounded-xl border-2 border-slate-200 peer-checked:border-blue-500 peer-checked:bg-blue-50 transition-all text-center">
                  <div className="font-bold text-slate-700 peer-checked:text-blue-700 mb-1">RETURN (From Site)</div>
                  <div className="text-xs text-slate-500">Receiving equipment back from a project</div>
                </div>
              </label>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Invoice Number (NF)</label>
              <div className="relative">
            ...
            <div className="col-span-full">
              ...
              <textarea {...register("notes")} rows={2} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 outline-none" placeholder="Optional notes..." />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-lg text-slate-900">Items</h3>
            <button 
              type="button" 
            ...
            {fields.map((field, index) => (
              <div key={field.id} className="flex gap-4 items-start bg-slate-50 p-4 rounded-xl border border-slate-100 animate-in fade-in slide-in-from-top-2">
                <div className="flex-1">
                  <label className="block text-xs font-medium text-slate-500 mb-1">Equipment</label>
                  <select 
                  ...
                  >
                    ...
                    {activeEquipments.map(e => (
                      <option key={e.id} value={e.id}>
                        {e.sku} - {e.name} 
                        {movementType === 'OUT' ? ` (Avail: ${e.available})` : ''}
                      </option>
                    ))}
                  </select>
                  {errors.items?.[index]?.equipmentId && <p className="text-rose-500 text-xs mt-1">Required</p>}
                </div>
                <div className="w-32">
                  ...
                  />
                  {errors.items?.[index]?.quantity && <p className="text-rose-500 text-xs mt-1">Invalid</p>}
                </div>
                <button 
                  type="button" 
        ...
        {createMovement.isError && (
          <div className="bg-rose-50 text-rose-600 p-4 rounded-xl border border-rose-200">
            Error: {createMovement.error.message}
          </div>
        )}
        ...
        <div className="flex justify-end gap-4">
          ...
          >
            {createMovement.isPending ? "Processing..." : "Confirm Movement"}
          </button>
        </div>
      </form>
    </div>
